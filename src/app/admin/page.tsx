...
export default function AdminPage() {
  ...
  const inputClass =
    "w-full bg-[#181818] border border-[#333333] rounded-lg px-4 py-2.5 text-sm text-[#d4d4d4] font-sans placeholder-neutral-600 focus:outline-none focus:border-indigo-400 focus:bg-[#252526] transition-colors";
      ...
      <div className="min-h-screen bg-[#0f172a] flex items-center justify-center px-4 font-sans text-[#d4d4d4]">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleAuth} className="space-y-4 bg-[#181818] border border-[#333333]/60 rounded-xl p-8 shadow-xl shadow-black/30">
            ...
            <button
              type="submit"
              className="w-full bg-indigo-500/10 border border-indigo-500/30 hover:bg-indigo-500/20 hover:border-indigo-400/50 text-indigo-400 font-semibold text-sm py-2.5 rounded-lg transition-all shadow-md shadow-indigo-500/5 mt-2"
            >
              Verify Credentials
            </button>
          </form>
    ...
    <div className="min-h-screen bg-[#0f172a] text-[#d4d4d4] font-sans">
      ...
      <nav className="sticky top-0 z-30 border-b border-[#333333]/50 bg-[#0f172a]/90 px-6 py-4 backdrop-blur-md">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          ...
          <span className="text-xs font-semibold uppercase tracking-wider text-indigo-400 bg-indigo-500/5 border border-indigo-500/10 px-3 py-1 rounded-full">
            Publishing Portal
          </span>
        </div>
      </nav>
      ...
      <div className="max-w-3xl mx-auto px-6 py-12 lg:py-16">
        ...
        {/* Success Banner */}
        {status === "success" && (
          <div className="flex items-center gap-3 bg-indigo-500/10 border border-indigo-500/30 rounded-lg px-4 py-3 mb-8 text-sm text-indigo-300">
            <CheckCircle className="h-4 w-4 flex-shrink-0" />
            <span>Post published successfully to production database!</span>
            <button
              onClick={() => setStatus("idle")}
              className="ml-auto text-xs text-indigo-400 hover:underline font-semibold"
            >
              Dismiss
            </button>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-[#181818]/60 border border-[#333333]/40 rounded-xl p-6 lg:p-8 shadow-xl shadow-black/5">
          ...
          <div>
            ...
            <p className="mt-2 text-[11px] text-neutral-500">
              Your article will be live at: <span className="text-indigo-400">/blog/{form.slug || "..."}</span>
            </p>
          </div>
          ...
          <div className="border-t border-[#333333]/50 pt-6 flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-indigo-500/10 border border-indigo-500/30 hover:bg-indigo-500/20 hover:border-indigo-400/50 text-indigo-400 font-semibold text-sm px-6 py-2.5 rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-md shadow-indigo-500/5"
            >
              {status === "loading" ? (
                <>
        ...
        <footer className="mt-16 border-t border-[#333333]/50 pt-6 text-xs text-neutral-500">
          <p>Database Source: <span className="text-indigo-400/80 font-medium">Supabase Cloud</span></p>
        </footer>
      </div>
    </div>
  );
